...
// Gets all accounts table
const getAllAccounts = async (req, res) => {
  const client = await pool.connect();

  client.query('SELECT * FROM accounts', (err, results) => {
    if (err) {
...
const getAccountById = async (req, res) => {
  ...
  await client.query('SELECT * FROM accounts WHERE account_id = $1', [account_id], (err, result) => {
    if (err) {
...
const getTotalBalanceOfAccounts = async (req, res) => { //FOR USERS with more than 1 account
    const client = await pool.connect();
